...
export class Vaus {
    ...
    public y: Fx = toFx(224); // fixed near bottom (can descend during exit warp)
    ...
    public reduceActive: boolean = false;
    ...
    public reduce(): void {
        if (this.reduceActive) return;
        this.reduceActive = true;
        this.enlargeActive = false;
        
        const oldWidth = this.width;
        this.width = toFx(16); // 16 px reduced
