...
export class RoundStateTracker {
    ...
    public megaActive: boolean = false;
    public isRoundCompletedMode: boolean = false;

    // Warp exit coordinates (left and right walls)
    public readonly warpLeftExitAABB: AABB = {
        x: toFx(8),
        y: toFx(208),
        w: toFx(12),
        h: toFx(24)
    ...
    public readonly warpRightExitAABB: AABB = {
        x: toFx(172),
        y: toFx(208),
        w: toFx(12),
        h: toFx(24)
    ...
    public update(input: { left: boolean; right: boolean; fire: boolean; pointerXDelta: number; pointerXAbsolute: number }, inputMode: string): void {
        ...
        if (this.isBreakWarpOpen) {
            ...
            if (isOverlapping(vausAABB, this.warpLeftExitAABB)) {
                EventBus.emit(GameEvents.BREAK_WARP_ENTERED, { branch: 'L' });
                return;
            }
            if (isOverlapping(vausAABB, this.warpRightExitAABB)) {
                EventBus.emit(GameEvents.BREAK_WARP_ENTERED, { branch: 'R' });
                return;
            }
        }

        // 4. Ball-out / life-loss check
        if (this.balls.length === 0 && !this.isRoundCompletedMode) {
            // Trigger life loss
        ...
        // Stage clear check (if not already completed and it's a standard brick level)
        if (!this.isRoundCompletedMode && this.bricks.clearRequiredCount === 0 && this.levelData && this.levelData.type === 'brick') {
            this.isRoundCompletedMode = true;
            this.isBreakWarpOpen = true;
            this.balls = []; // Clear balls so they disappear
    ...
    private applyPowerUp(type: CapsuleType): void {
        ...
        switch (type) {
            ...
            case 'E':
                ...
                break;
            case 'B':
                // Break Exit does not open on final Round 33 or boss Round 34
                if (GameState.currentRoundNum < 33) {
                    this.isBreakWarpOpen = true;
                    EventBus.emit(GameEvents.BREAK_WARP_OPENED);
                }
                break;
            ...
            case 'M':
                ...
                EventBus.emit(GameEvents.POWERUP_ACTIVATED, { type: 'M' });
                break;
            case 'R':
                this.vaus.reduce();
                EventBus.emit(GameEvents.POWERUP_ACTIVATED, { type: 'R' });
                break;
        }
    }
    ...
    private resolveBallPhysics(): void {
        ...
        for (const ball of [...this.balls]) {
            ...
            if (candidates.length > 0) {
                ...
                // Reflect velocity ONLY IF NOT MEGA BALL
                if (!this.megaActive) {
